                 ...
                 tabPanel("Electricity use by county",
                          ...
                          sidebarLayout(
                            sidebarPanel("Some text!",
                                         checkboxGroupInput(inputId = "county_selection",
                                                            "Choose some!",
                                                            choices = c(levels(solar_capacity_df$county)))
                                         ),
                            mainPanel("Main panel text!",
...
server <- function(input, output){
  ...
  solar_capacity_df <- plants_location_join %>% 
    ...
    mutate(plant_name = fct_reorder(plant_name))
  ...
  output$solar_capacity_plot <- renderPlot({
     solar_capacity_plot <- ggplot(data = solar_capacity_df(),
      ...
      scale_x_continuous(lim = c(2008,2019),
                         expand = c(0,0),
