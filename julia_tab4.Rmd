...
# Get total minus solar for each state by putting into wide format
total_minus_solar_state <- solar_by_state_2018 %>% 
  pivot_wider(names_from = energy_source, 
              values_from = energy_source_sum) %>% 
  clean_names() %>% 
  mutate(solar_thermal_and_photovoltaic = 
           replace_na(solar_thermal_and_photovoltaic, 0)) %>% 
  mutate(total_minus_solar = (total - solar_thermal_and_photovoltaic))

# put back into longer format for ggplot
total_minus_solar_longer <- total_minus_solar_state %>% 
  pivot_longer(cols = c(total, solar_thermal_and_photovoltaic, total_minus_solar)) %>% 
  filter(name %in% c("solar_thermal_and_photovoltaic", "total_minus_solar")) %>% 
  filter(state == "CA")

# Make graph of solar compared to all other energy for CA in 2018

ggplot(data = total_minus_solar_longer, 
       aes(x = state, 
           y = value, 
           fill = name)) +
